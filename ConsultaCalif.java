...
public class ConsultaCalif extends JFrame {
    private JTextArea areaMostrarRegistros; // Área para mostrar los registros
    private JButton botonAbrir, botonCredito, botonDebito, botonCeros, botonSalir; // Botones de la interfaz
    private JPanel panelBotones; // Panel para colocar los botones
    private ObjectInputStream entrada; // Flujo de entrada para leer objetos
    private FileInputStream entradaArchivo; // Flujo de entrada para el archivo
    private File nombreArchivo; // Archivo seleccionado
    private String tipoCuenta; // Tipo de cuenta seleccionado por el usuario

    // configurar GUI
    public ConsultaCalif() {
        super("Programa de consulta de Calificaciones");

        // Obtener el contenedor para agregar componentes
        Container contenedor = getContentPane();
        panelBotones = new JPanel(); // configurar panel para agregarle botones

        // crear y configurar botón para abrir el archivo
        ...
        // crear y configurar botón para salir del programa
        ...
        // deshabilitar botones de consulta hasta que se abra un archivo
        botonCredito.setEnabled(false);
        botonDebito.setEnabled(false);
    ...
    private void abrirArchivo() {
        ...
        // abrir el archivo
        try {
            // cerrar archivo de la operación anterior si es necesario
            if (entrada != null)
                entrada.close();
    ...
    private void leerRegistros() {
        ...
        try {
            ...
            while (true) {
                ...
                if (debeMostrarse(registro.ObtenerProm()))
                    areaMostrarRegistros.append(
                        registro.ObtenerNumeroDeControl() + "\t" + 
                        registro.ObtenerNombre() + "\t" + "\t" + 
                        (registro.ObtenerProm()) + "\t" + 
                        registro.ObtenerCalif1() + "\t" + 
                        registro.ObtenerCalif2() + "\t" + 
                        registro.ObtenerCalif3() + "\n");
            }
        } // fin del bloque try
